...
<div class=modal-subtitle>Saved to Supabase</div>
</div>
<button class="btn btn-ghost" onclick="closeModal('modal-location')">
...
<input type="text" id="f-name" placeholder="e.g. Silo District Market">
</div>
</div>
<div class="form-row">
<div class="field">
<label>City <span style="color:var(--danger)">*</span></label>
<select id="f-city" onchange="toggleNewCity(this)">
<option value="">Select a city…</option><option value="__new__">+ Add new city…</option>
</select>
<input type="text" id="f-new-city" placeholder="Enter new city name…" style="display:none;margin-top:6px" class="form-input">
</div>
<div class="field">
...
<textarea id="f-desc" rows="3" placeholder="Brief description of this location…"></textarea>
</div>
</div>
</div>
<div class="form-section">
...
<input type="text" class="mono" id="f-lat">
<span class="field-hint">N positive</span>
</div>
<div class="field">
<label>Longitude <span style="color:var(--danger)">*</span></label>
<input type="text" class="mono" id="f-lng">
<span class="field-hint">W negative</span>
</div>
</div>
</div>
<div class="form-section">
...
Contact & Web
<span class="optional-tag">opt.</span>
</div>
<div class="form-row">
<div class="field">
<label>Website</label>
...
<div class="field">
<label>Phone</label>
<input type="tel" id="f-phone" placeholder="(228) 555-0123">
</div>
</div>
</div>
<div class="form-section">
...
<div class="img-tabs">
<button class="img-tab active" onclick="switchImgTab('profile','upload',this)">Upload File</button>
<button class="img-tab" onclick="switchImgTab('profile','url',this)">Paste URL</button>
</div>
<div id="profile-upload-panel">
<div class="img-drop-zone" id="profile-drop-zone">
<input type="file" id="profile-file-input" accept="image/jpeg,image/png,image/webp,image/gif" onchange="handleProfileFile(this.files[0])">
<div class="img-drop-title">Drop image here or click to browse</div>
<div class="img-drop-sub">JPG, PNG, WebP or GIF · max 10MB</div>
</div>
<div id="profile-preview" style="display:none"></div>
</div>
<div id="profile-url-panel" style="display:none">
<div class="field">
<input type="url" id="f-profile-url">
<span class="field-hint">Direct link.</span>
</div>
</div>
</div>
</div>
...
<input type="file" accept="image/jpeg,image/png,image/webp,image/gif" multiple onchange="handleGalleryFiles(this.files)">
<svg fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M12 4v16m8-8H4"/></svg>
</div>
</div>
<div style="margin-top:8px">
<div class="field">
<input type="url" id="f-gallery-url" placeholder="Or paste an image URL and press Enter">
...
<div class="form-row single">
<div class="field">
<label>Speechify Link</label>
<input type="url" id="f-speechify">
<span class="field-hint">Speechify share link.</span>
</div>
</div>
</div>
<div class="form-section">
<div class=form-section-title>Settings</div>
<div class="focal-toggle-row">
<div>
<div class="focal-toggle-label">Focal Point</div>
<div class="focal-toggle-hint">Highlights this location as a featured stop in the city tour.</div>
</div>
<label class="toggle">
<input type="checkbox" id="f-focal">
...
<div class="pdot" id="pd-mon" style="background:var(--gray-100);color:var(--gray-400)">M</div>
<div class="pdot" id="pd-tue" style="background:var(--gray-100);color:var(--gray-400)">T</div>
<div class="pdot" id="pd-wed" style="background:var(--gray-100);color:var(--gray-400)">W</div>
<div class="pdot" id="pd-thu" style="background:var(--gray-100);color:var(--gray-400)">T</div>
<div class="pdot" id="pd-fri" style="background:var(--gray-100);color:var(--gray-400)">F</div>
<div class="pdot" id="pd-sat" style="background:var(--gray-100);color:var(--gray-400)">S</div>
<div class="pdot" id="pd-sun" style="background:var(--gray-100);color:var(--gray-400)">S</div>
</div>
<div class="hours-editor" id="hours-editor"></div>
<div class="quick-fill">
<span class="quick-label">Quick fill:</span>
<button class="chip" onclick="copyMonToAll()">Copy Mon → all open</button>
<button class="chip" onclick="setPattern('weekdays')">Weekdays</button>
<button class="chip" onclick="setPattern('all7')">All 7 days</button>
<button class="chip" onclick="setPattern('weekend')">Weekends</button>
<button class="chip" onclick="setPattern('none')">Clear all</button>
</div>
</div>
</div>
<div class=modal-footer>
<button class="btn btn-secondary" onclick="closeModal('modal-location')">Cancel</button>
<button class="btn btn-primary" id="btn-save-location" onclick="saveLocation()">
Save Location
</button>
</div>
</div>
...
<div class=modal-header>
<div>
<div class=modal-title>Import via CSV</div>
<div class=modal-subtitle>Bulk-add locations from a client spreadsheet.</div>
</div>
<button class="btn btn-ghost" onclick="closeModal('modal-import')">
<svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M6 18L18 6M6 6l12 12"/></svg>
</button>
</div>
<div class=modal-body>
...
<svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/></svg>
<div class="import-zone-title">Drop CSV or click to browse</div>
<div class="import-zone-sub">Max 500 rows</div>
</div>
<div class="csv-template">
<div class="csv-template-title" style="display:flex;align-items:center;justify-content:space-between">
CSV column reference
<button class="btn btn-secondary" style="padding:4px 10px;font-size:11.5px" onclick="downloadTemplate()">
<svg fill="none" viewBox="0 0 24 24" stroke="currentColor" style="width:12px;height:12px"><path d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
Download template
</button>
</div>
<div class="csv-cols" style="margin-top:10px">
<span class="csv-col required">name *</span><span class="csv-col required">city *</span><span class="csv-col required">latitude *</span><span class="csv-col required">longitude *</span><span class="csv-col">category</span><span class="csv-col">address</span><span class="csv-col">description</span><span class="csv-col">website</span><span class="csv-col">phone</span><span class="csv-col">profile_image</span><span class="csv-col">audio_file_link</span><span class="csv-col">is_focal_point</span><span class="csv-col">hours_monday</span><span class="csv-col">hours_tuesday</span><span class="csv-col">hours_wednesday</span><span class="csv-col">hours_thursday</span><span class="csv-col">hours_friday</span><span class="csv-col">hours_saturday</span><span class="csv-col">hours_sunday</span>
</div>
<div style="margin-top:10px;font-size:11.5px;color:var(--gray-400);display:flex;flex-direction:column;gap:3px">
<span>Hours format: <span style="font-family:'DM Mono',monospace">9:00 AM - 5:00 PM</span> or <span style="font-family:'DM Mono',monospace">Closed</span></span>
<span>is_focal_point: <span style="font-family:'DM Mono',monospace">TRUE</span> or <span style="font-family:'DM Mono',monospace">FALSE</span></span>
<span>City must match exactly: <span style="font-family:'DM Mono',monospace" id="import-city-hint">—</span></span>
</div>
</div>
</div>
...
<div class=modal-footer>
<button class="btn btn-secondary" onclick="resetImportModal()">
<svg fill="none" viewBox="0 0 24 24" stroke="currentColor" style="width:13px;height:13px"><path d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
Back
</button>
<button class="btn btn-secondary" onclick="closeModal('modal-import')">Cancel</button>
<button class="btn btn-primary" id="btn-import-confirm" style="display:none" onclick="confirmImport()">
<svg fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>
<span id="btn-import-label">Import</span>
</button>
</div>
</div>
</div>
...
(function(){const s=getSession();if(!s||!s.access_token){window.location.href=LOGIN_URL;return;}if(s.expires_at&&Date.now()>s.expires_at-60000){clearSession();window.location.href=LOGIN_URL;}})();
...
function showToast(msg, type = 'success') {
  ...
  const icon = type === 'success'
    ? '<svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M5 13l4 4L19 7"/></svg>'
...
function normalizeHours(raw) {
  ...
  for (const [key, val] of Object.entries(raw)) {
    ...
    if (openStr && closeStr) out[shortKey] = { open: openStr, close: closeStr };
  }
  return out;
}
function denormalizeHours(hours) {
  ...
  const out = {};
  for (const [short, val] of Object.entries(hours)) {
    const full = DAY_SHORT_TO_FULL[short];
...
function to24h(t) {
  if (!t || t === 'Closed' || t === 'Open 24 Hours') return null;
  if (!t.includes('AM') && !t.includes('PM')) return t; // already 24h
...
function renderTable(locations) {
  ...
  tbody.innerHTML = filtered.map(l => {
    ...
    const DOT_KEYS = ['mon','tue','wed','thu','fri','sat','sun'];
    const dotsHtml = DOT_KEYS.map((d, i) => {
      const day   = hours[d];
      const is24  = day && day.open === '00:00' && day.close === '23:59';
      const cls   = !day ? 'closed' : is24 ? 'allday' : 'open';
      const tip   = !day ? 'Closed' : is24 ? 'Open 24 hrs' : `${formatTime(day.open)}–${formatTime(day.close)}`;
...
function applySortFilter(locs){
  ...
  let f=locs.filter(l=>{
    const mc=!cf||l.city_id===cf;
    const cn=getCityName(l.city_id).toLowerCase();
    return mc&&(!s||l.name?.toLowerCase().includes(s)||l.address?.toLowerCase().includes(s)||cn.includes(s));
  });
  f.sort((a,b)=>{
    let av='',bv='';
    ...
    else if(_sortKey==='category'){av=getCatName(a.category_id)?.toLowerCase()||'';bv=getCatName(b.category_id)?.toLowerCase()||'';}
    else if(_sortKey==='updated'){av=a.updated_at||'';bv=b.updated_at||'';}
    return av<bv?(_sortDir==='asc'?-1:1):av>bv?(_sortDir==='asc'?1:-1):0;
  });
  return f;
}
function filterTable(){window._allLocations&&renderTable(window._allLocations);}
async function deleteLocation(id) {
  if(!confirm('Delete this location? '))return;
  try {
    await sbFetch(`locations?id=eq.${id}`, {method:'DELETE',prefer:'return=minimal'});
...
function addGalleryItem(url, label) {
  ...
  item.innerHTML = `
    ...
    <button class="gallery-item-remove" onclick="removeGalleryItem(this,'${url}')">
      <svg fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M6 18L18 6M6 6l12 12"/></svg>
    </button>`;
...
const DAYS = [
  ...
  { id:'thu', label:'Thursday' },{ id:'fri', label:'Friday' },
  { id:'sat', label:'Saturday' },{ id:'sun', label:'Sunday' },
];
...
function updateDot(day) {
  ...
  if (!dot) return;
  if (!tog.checked) {
    dot.style.background = 'var(--gray-100)'; dot.style.color = 'var(--gray-400)';
  } else if (open.value === '00:00' && close.value === '23:59') {
    dot.style.background = 'var(--accent-light)'; dot.style.color = 'var(--accent)';
  } else {
    dot.style.background = 'var(--success-light)'; dot.style.color = 'var(--success)';
  }
}
...
function setPattern(pattern) {
  ...
  const wkn = ['sat','sun'];
  DAYS.forEach(d => {
    const isOpen = pattern === 'all7' ? true
      : pattern === 'weekdays' ? wkd.includes(d.id)
      : pattern === 'weekend'  ? wkn.includes(d.id)
      : false;
...
function openAddModal() {
  ...
  resetImageState();
  setPattern('none');
  gid('modal-location').classList.add('open');
...
async function openEditModal(id) {
  try {
    ...
    resetImageState();
    if (loc.profile_image) {
      setProfilePreview(loc.profile_image);
    }
    if (loc.images && loc.images.length) {
      loc.images.forEach(url => addGalleryItem(url, ''));
    }
...
async function handleImportFile(file) {
  ...
  const processed = rows.map((row, i) => {
    ...
    return { rowNum, name, cityName, cityId: cityObj?.id || null, catId: catObj?.id || null, status: 'add', row };
  });
...
function showImportPreview(rows, filename) {
  ...
  gid('import-preview-content').innerHTML =
    summaryHtml + `<div class="import-preview-table">${headerHtml}${rowsHtml}</div>`;
...
async function confirmImport() {
  ...
  for(const r of rows){try{
    ...
    const cityId=r.cityId||await getOrCreateCity(r.cityName,+r.row['latitude'],+r.row['longitude']);
      const rr=r.row;const payload={name:r.name,slug:generateSlug(r.name),city_id:cityId,category_id:r.catId||null,address:rr['address']||null,description:rr['description']||null,latitude:parseFloat(rr['latitude']),longitude:parseFloat(rr['longitude']),website:rr['website']||null,phone:rr['phone']||null,profile_image:rr['profile_image']||null,audio_file_link:rr['audio_file_link']||null,is_focal_point:(rr['is_focal_point']||'').toUpperCase()==='TRUE',operating_hours:hoursFromCSVRow(rr),created_at:now,updated_at:now};
    await sbFetch('locations',{method:'POST',body:JSON.stringify(payload)});
    added++;
  }catch(e){failed++;}}
  btn.disabled=false;
  closeModal('modal-import');
  loadAndRenderTable();
  showToast(failed===0?`${added} location${added!==1?'s':''} imported successfully.`:`${added} imported, ${failed} failed.`,failed>0?'error':'success');
}
...
async function saveLocation() {
  ...
  if (!cityVal && !newCityInput) { showToast('Please select or enter a city.', 'error'); return; }
  if (isNaN(lat) || isNaN(lng)) { showToast('Valid coordinates are required.', 'error'); return; }
  const resolvedCityId = newCityInput
    ? await getOrCreateCity(newCityInput)
    : cityVal;
...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  gid('f-gallery-url')?.addEventListener('keydown', e => {
    if (e.key === 'Enter') {
      ...
      if (url) { addGalleryItem(url, url); e.target.value = ''; }
    }
  });
