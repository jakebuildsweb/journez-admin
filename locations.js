      ...
      <div id="import-step-upload">
        ...
        <div class="csv-template">
          ...
          <div class="csv-cols" style="margin-top:10px">
            <span class="csv-col required">name *</span>
            <span class="csv-col required">city *</span>
            <span class="csv-col required">latitude *</span>
            <span class="csv-col required">longitude *</span>
            <span class="csv-col">category</span>
            <span class="csv-col">address</span>
            <span class="csv-col">description</span>
            <span class="csv-col">website</span>
            <span class="csv-col">phone</span>
            <span class="csv-col">profile_image</span>
            <span class="csv-col">audio_file_link</span>
            <span class="csv-col">is_focal_point</span>
            <span class="csv-col">hours_monday</span>
            <span class="csv-col">hours_tuesday</span>
            <span class="csv-col">hours_wednesday</span>
            <span class="csv-col">hours_thursday</span>
            <span class="csv-col">hours_friday</span>
            <span class="csv-col">hours_saturday</span>
            <span class="csv-col">hours_sunday</span>
          </div>
...
function updateSidebar(count) {
  const b = document.getElementById('nav_badge');
  if (b) b.textContent = count;

  sbFetch('events?select=id')
    .then(r => {
      const e = document.getElementById('nav_badge_events');
      if (e) e.textContent = (r || []).length;
    })
    .catch(() => {});
}
...
function updateLastUpdated(iso) {
  ...
  document.querySelectorAll('[class*="card_component"]').forEach(card => {
    ...
    if (lbl && lbl.textContent.trim().toUpperCase().includes('LAST UPDATED')) {
      const v = card.querySelector('[class*="card_value"]');
      const s = card.querySelector('[class*="card_sub"]');
      if (v) v.textContent = today ? 'Today' : ds;
      if (s) s.textContent = today ? ts : `${ds}, ${ts}`;
    }
  });
}
...
async function loadAndRenderTable() {
  ...
  try {
    const locations = await sbFetch(
      'locations?select=id,name,address,latitude,longitude,city_id,category_id,profile_image,audio_file_link,is_focal_point,operating_hours,slug,updated_at,website,phone,description,images&order=name'
    );
...
function renderTable(locations) {
  ...
  tbody.innerHTML = filtered.map(l => {
    ...
    return `<div class="table-row" onclick="openEditModal('${l.id}')">
      <div>
        <div style="display:flex;align-items:center;gap:6px">
          <div class="loc-name">${l.name}</div>${focalDot}${audioIcon}
        </div>
        <div class="loc-addr">${l.address || '—'}</div>
      </div>
      <div><span class="city-tag">${cityName}</span></div>
      <div>${catTag}</div>
      <div>${hoursHtml}</div>
      <div class="row-actions">
        <button class="icon-btn" onclick="event.stopPropagation();openEditModal('${l.id}')">
          <svg fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.4-9.4a2 2 0 112.8 2.8L11.8 15H9v-2.8l8.6-8.6z"/></svg>
        </button>
        <button class="icon-btn danger" onclick="event.stopPropagation();deleteLocation('${l.id}')">
          <svg fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
        </button>
      </div>
    </div>`;
  }).join('');
  ...
  gid('hours-editor').innerHTML = DAYS.map(d => `
    <div class="hours-row">
      ...
      <div class="hours-controls">
        <label class="toggle">
          <input type="checkbox" id="tog-${d.id}" onchange="toggleDay('${d.id}',this.checked)">
          <span class="toggle-track"></span>
        </label>
        <div class="hours-time-group" id="times-${d.id}" style="display:none">
          <input type="time" class="time-input" id="open-${d.id}" value="09:00" onchange="onTimeChange('${d.id}')">
          <span class="time-sep">to</span>
          <input type="time" class="time-input" id="close-${d.id}" value="17:00" onchange="onTimeChange('${d.id}')">
          <button class="chip" id="chip24-${d.id}" onclick="set24('${d.id}')">24 hrs</button>
        </div>
        <div class="hours-closed-label" id="closed-${d.id}">Closed</div>
      </div>
    </div>
  `).join('');
}
...
function hoursFromCSVRow(row) {
  ...
  CSV_DAY_COLS.forEach(day => {
    ...
    if (!val || val.toLowerCase() === 'closed') return;
...
async function saveLocation() {
  ...
  const resolvedCityId = newCityInput
    ? await getOrCreateCity(newCityInput, lat, lng)
    : cityVal;
