...
- An unmatched CSV city name does not error on import — but it does **not** null out either. `handleImportFile` (`locations.js:829`) leaves `cityId: null` on the preview row, then `confirmImport` (`locations.js:902`) falls through to `getOrCreateCity()` (`locations.js:205`), which POSTs a **brand-new city** using that location's own lat/lng as the city centre. A typo (`Biloxi, Ms` vs `Biloxi`) permanently adds a near-duplicate to the `cities` table and the city filter. It also bypasses dedup: `isDupe` only fires when `cityObj` matched, so mistyped rows re-import as duplicates every time. See the importer contract in `../assets/data/AGENTS.md`.
- Because of that, the `city` value in any hand-written CSV must be an exact live city name. `docs/journez_template.csv` is a static snapshot — its example row currently matches (`Biloxi`, verified 2026-08-05), but the in-app `downloadTemplate()` builds from live data and can't drift.
